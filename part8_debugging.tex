...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Image Format (1/2)}
    \begin{block}{Symptom}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Image Format (2/2)}
    ...
    \begin{itemize}
        ...
        \item Float images instead of uint8
    \end{itemize}

    \vspace{0.5cm}
    \begin{alertblock}{Quick Check}
        \begin{lstlisting}[language=Python,basicstyle=\ttfamily\tiny]
print(f"Shape: {img.shape}")  # Should be [384, 768, 3]
print(f"dtype: {img.dtype}")  # Should be uint8
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Normalizer Mismatch (1/2)}
    \begin{block}{Symptom}
...
%------------------------------------------------------------------------------
\begin{frame}{Common Issue: Normalizer Mismatch (2/2)}
    \textbf{Solution:}
    \begin{itemize}
        \item Use normalizer trained on same data distribution
        \item Create new normalizer from ManiSkill dataset
        \item Verify normalizer mode matches training
    \end{itemize}

    \vspace{0.5cm}
    \begin{alertblock}{Verification}
        After normalization, values should be in $[-1, 1]$ range for "limits" mode
    \end{alertblock}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Token Conversion}
    \begin{block}{Symptom}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Rotation Format (1/2)}
    \begin{block}{Symptom}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Common Issue: Rotation Format (2/2)}
...
det = torch.det(R)           # Should be 1
    ...
    \vspace{0.5cm}
    \begin{alertblock}{Common Rotation Mistakes}
        \begin{itemize}
            \item Using quaternion instead of 6D representation
            \item Wrong dimension ordering in 6D
            \item Missing Gram-Schmidt orthonormalization
        \end{itemize}
    \end{alertblock}
\end{frame}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Debugging Tool: Action Visualization (1/2)}
    \begin{lstlisting}[language=Python]
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Debugging Tool: Action Visualization (2/2)}
    \begin{lstlisting}[language=Python]
    # Gripper (right arm)
    ...
    \vspace{0.5cm}
    \begin{block}{Usage Tip}
        Compare visualized trajectories to expected motion for quick sanity check
    \end{block}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{Debugging Tool: Step-by-Step Trace}
    \begin{lstlisting}[language=Python]
def debug_inference(model, vae, normalizer, obs, instruction):
    ...
    # 3. Token conversion + validation
    vae_tokens = convert_to_vae(tokens)
    valid = (vae_tokens >= 0) & (vae_tokens < 1024)
    print(f"Valid tokens: {valid.sum()}/{len(valid)}")

    # 4. Decode + unnormalize
    action = normalizer["action"].unnormalize(vae.decode(vae_tokens))
...
%------------------------------------------------------------------------------
\begin{frame}{ManiSkill Adaptation: Action Space (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{ManiSkill Adaptation: Action Space (2/2)}
...
def convert_action(rdt2_action, env):
    ...
    \begin{alertblock}{Key Point}
        Each ManiSkill environment may require different action conversion
    \end{alertblock}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{ManiSkill Adaptation: Franka Setup}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Creating Custom Normalizer}
