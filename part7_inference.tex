...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{inference\_real\_vq.py: Arguments}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{inference\_real\_fm.py: Arguments}
    \begin{lstlisting}[language=Python]
@click.command()
...
\begin{frame}{Complete FM Inference Pipeline}
    ...
    \begin{tikzpicture}[
        ...
        % Row 1: Inputs
        \node[box, fill=blue!20] (cam) at (0,3) {Cameras};
        \node[box, fill=green!20] (inst) at (2.5,3) {Instruction};
        \node[box, fill=gray!20] (noise) at (6,3) {Noise\\$\mathcal{N}(0,I)$};
        \node[box, fill=actionorange!30] (state) at (9,3) {Robot State};

        % Row 2: Processing
        \node[box, fill=qwenblue!30] (qwen) at (1.25,1) {Qwen2.5-VL\\(1 forward)};
        \node[box, fill=rdtgreen!30] (rdt) at (6,1) {RDT Expert\\(5 forwards)};

        % Row 3: Intermediate
        \node[box, fill=yellow!30] (kv) at (1.25,-1) {KV Cache};
        \node[box, fill=vqpurple!30] (action) at (6,-1) {Normalized\\Action};

        % Row 4: Post-processing
        \node[box, fill=gray!20] (unnorm) at (9,-1) {Unnormalize};
        \node[box, fill=gray!20] (grip) at (11.5,-1) {Gripper\\Rescale};
        \node[box, fill=actionorange!30] (final) at (14,-1) {Robot\\Action};
        ...
        % KV Cache to RDT
        \draw[arrow] (kv.east) -- ++(1.2,0) |- (rdt.west);
        ...
        \draw[arrow] (grip) -- (final);

        % Frozen indicator
        \node[draw, dashed, blue, thick, fit=(qwen), inner sep=2pt, label={[font=\tiny, blue]below:Frozen}] {};
    \end{tikzpicture}
    \end{center}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{Model Loading (VQ)}
    \begin{lstlisting}[language=Python]
# Load processor
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Model Loading (FM)}
    \begin{lstlisting}[language=Python]
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{batch\_predict\_action Function}
    \textbf{File:} \code{utils.py}

    \vspace{0.2cm}
    \begin{lstlisting}[language=Python]
def batch_predict_action(model, processor, vae, ...):
    # 1. Preprocess + build chat template
    ...
    # 3. Convert VLA -> VAE and decode
    vae_ids = vocab_size - (action_ids + 1)
    vae_ids = vae_ids.clamp(0, 1023)
    actions = vae.decode(vae_ids)
    return normalizer["action"].unnormalize(actions)
    \end{lstlisting}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{RDTInferencer.step() (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{RDTInferencer.step() (2/2)}
    \begin{lstlisting}[language=Python]
    ...
    \begin{block}{Key Point}
        Instruction embedding is cached to skip VLM forward for repeated instructions
    \end{block}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}{Latency Constants (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Latency Constants (2/2)}
...
frame_latency = 1/60           # ~16.7ms
    ...
    \vspace{0.5cm}
    \begin{block}{Total Latency}
        $L_{\text{total}} = L_{\text{camera}} + L_{\text{inference}} + L_{\text{exec}}$
    \end{block}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}{Latency Compensation (1/2)}
    \begin{block}{Problem}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Latency Compensation (2/2)}
    \begin{lstlisting}[language=Python]
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Gripper Rescaling}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Binarize Gripper Option}
    \begin{block}{Purpose}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{UR5e Controller (RTDE)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Franka FR3 Controller (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Franka FR3 Controller (2/2)}
    \begin{lstlisting}[language=Python]
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{BimanualUmiEnv}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Observation Structure}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Action Execution}
    \begin{lstlisting}[language=Python]
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Control Loop Structure}
    \begin{lstlisting}[language=Python]
while True:
    obs = env.get_obs()
    t_obs = time.time()
    ...
    # Gripper rescaling (UMI -> robot)
    actions[:, 9] = actions[:, 9] / 0.088 * 0.10
    actions[:, 19] = actions[:, 19] / 0.088 * 0.10

    # Execute with latency compensation
    timestamps = compute_action_timestamps(t_obs, inference_time)
    env.exec_actions(actions, timestamps, compensate_latency=True)
    \end{lstlisting}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{Action Space Conversion}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{TCP Space Conversion (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}{TCP Space Conversion (2/2)}
    \begin{alertblock}{Important}
        This conversion is critical for accurate robot control. Incorrect TCP offset causes systematic errors.
    \end{alertblock}

    \vspace{0.5cm}
    \begin{block}{Calibration Required}
        The \code{tx\_tracker\_to\_tcp} matrix must be calibrated for each robot setup using the calibration scripts
    \end{block}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}[fragile]{Robot Configuration YAML (1/2)}
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{Robot Configuration YAML (2/2)}
    \textbf{Robot and Gripper Configuration:}
    \begin{lstlisting}[language=Python]
robots:
...
%------------------------------------------------------------------------------
\begin{frame}[fragile]{vLLM Inference (2/2)}
    \begin{lstlisting}[language=Python]
# Sampling parameters
...
outputs = llm.generate(prompts, sampling_params)
    ...
    \vspace{0.5cm}
    \begin{block}{vLLM Advantages}
        \begin{itemize}
            \item $\sim$1.7$\times$ speedup over HuggingFace
            \item Paged attention for efficient memory
            \item Only available for RDT2-VQ (discrete tokens)
        \end{itemize}
    \end{block}
\end{frame}
